...
def _read_video_decord(
    ...
    # Decord uses automatic multithreading when num_threads is 0. That is
    # useful for standalone decoding, but can oversubscribe the host during
    # multi-GPU training because every rank decodes independently.
    decord_num_threads = max(0, int(os.environ.get("DECORD_NUM_THREADS", "0")))
    video_path = ele["video"]
    st = time.time()
    vr = decord.VideoReader(video_path, num_threads=decord_num_threads)
